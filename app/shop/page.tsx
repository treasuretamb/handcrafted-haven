...
const CATEGORIES = ["All", "Decor", "Home", "Jewelry", "Kitchen", "Clothing", "Art", "Other"];
      ...
      <div className="max-w-7xl mx-auto px-6 py-10 flex flex-col md:flex-row gap-10">
        <aside className="md:w-64 shrink-0">
          <div className="bg-white rounded-xl shadow p-6 sticky top-6">
            ...
            <div className="mb-6">
              <label htmlFor="search" className="block text-sm font-semibold text-[var(--dark)] mb-2">Search</label>
              <input
                id="search"
                type="text"
                  ...
                  {filtered.map((product) => (
                    <Link key={product.id} href={`/shop/${product.id}`} className="group bg-white rounded-xl overflow-hidden shadow hover:shadow-xl transition">
                      ...
                      <div className="p-5">
                        ...
                        <p className="text-sm text-[var(--muted)]">by {product.seller_name}</p>
                        <div className="flex items-center justify-between mt-3">
                          <span className="text-[var(--primary)] font-bold text-lg">${Number(product.price).toFixed(2)}</span>
